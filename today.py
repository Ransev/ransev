...
USER_NAME = os.environ.get('USER_NAME', 'Ransev')
...
def graph_commits():
    """
    Uses GitHub's commit search API to return my indexed lifetime commit count.
    """
    ...
    raise Exception(graph_commits.__name__, ' has failed with a', request.status_code, request.text, QUERY_COUNT)
...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data):
    """
    Parse the SVG and update its age and GitHub statistics.
    """
    ...
    find_and_replace(root, 'follower_data', f'{follower_data:,}')
...
if __name__ == '__main__':
    ...
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2001, 1, 17))
